...
object SlickDao extends Dao {
  ...
  def getUser(name: String, password: String): Option[models.User] = db withDynTransaction {
    users.filter(u => u.name === name && u.password === password).firstOption map ModelConverter.getUser
  }

  def getUser(id: Long): Option[models.User] = db withDynTransaction {
    users.filter(_.id === id).firstOption map ModelConverter.getUser
  }

  def getUser(name: String): Option[models.User] = db withDynTransaction {
    users.filter(_.name === name).firstOption map ModelConverter.getUser
  }

  def getEntries(user: Option[models.User], page: Int, itemsOnPage: Int): (Long, Seq[models.Entry]) = db withDynTransaction {
  ...
  def getTag(title: String): Option[models.Tag] = db withDynTransaction {
    tags.filter(_.title === title).firstOption map ModelConverter.getTag
  }

  def getEntriesByTag(user: Option[models.User], tag: models.Tag, page: Int, itemsOnPage: Int): (Long, Seq[models.Entry]) = db withDynTransaction {
    val query = for {
      entry <- entries
  ...
  def getEntriesBySearch(user: Option[models.User], query: String, page: Int, itemsOnPage: Int): (Long, Seq[models.Entry]) = db withDynTransaction {
  ...
  def getEntriesBySearch(user: Option[models.User], query: String, from: Option[java.util.Date], to: Option[java.util.Date],
                         users: Seq[models.User], tags: Seq[models.Tag], page: Int, itemsOnPage: Int): (Long, Seq[models.Entry]) = db withDynTransaction {
  ...
  def getEntry(user: Option[models.User], id: Long): Option[models.Entry] = db withDynTransaction {
  ...
  def getTagsByTitles(titles: Seq[String], addNew: Boolean): Seq[models.Tag] = db withDynTransaction {
  ...
  def getTagsBySearch(query: String): Seq[models.Tag] = db withDynTransaction {
    tags.filter(_.title like "%" + query + "%").take(rowNumberInPopupSearch).list map ModelConverter.getTag
  }

  def getUsersByNames(names: Seq[String]): Seq[models.User] = db withDynTransaction {
    users.filter(_.name inSet names).take(rowNumberInPopupSearch).list map ModelConverter.getUser
  }

  def getUsersBySearch(query: String): Seq[models.User] = db withDynTransaction {
  ...
  def deleteEntry(user: Option[models.User], id: Long): Boolean = db withDynTransaction {
  ...
  def deleteComment(user: Option[models.User], id: Long): Boolean = db withDynTransaction {
  ...
  def addFavoriteTag(user: Option[models.User], title: String): Boolean = db withDynTransaction {
  ...
  def removeFavoriteTag(user: Option[models.User], title: String): Boolean = db withDynTransaction {
    val tag = getTag(title)
  ...
  private[slick] def getTagsByEntry(entryId: Long): Seq[models.Tag] = db withDynTransaction {
    val query = for {
      tag <- tags
      etr <- entryTagRelation
  ...
  private[slick] def getEntriesByUser(userId: Long): Seq[models.Entry] = db withDynTransaction {
    entries.filter(_.author === userId).list map ModelConverter.getEntry
  }

  private[slick] def getCommentsByUser(userId: Long): Seq[models.Comment] = db withDynTransaction {
    comments.filter(_.author === userId).list map ModelConverter.getComment
  }

  private[slick] def getFavoriteTagsByUser(userId: Long): Seq[models.Tag] = db withDynTransaction {
    val query = for {
      tag <- tags
  ...
  private[slick] def getCommentsByEntry(entryId: Long): Seq[models.Comment] = db withDynTransaction {
    comments.filter(_.entry === entryId).list map ModelConverter.getComment
  }

  private[slick] def getEntry(id: Long): Option[models.Entry] = db withDynTransaction {
    entries.filter(_.id === id).firstOption map ModelConverter.getEntry
  }

  private[slick] def getComment(id: Long): Option[models.Comment] = db withDynTransaction {
