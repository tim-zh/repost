...
object TestDao extends Dao {
  ...
  def getEntriesByTag(user: Option[models.User], tag: models.Tag, page: Int, itemsOnPage: Int): (Long, Seq[Entry]) = {
    require(itemsOnPage != 0)
  ...
  def getEntriesBySearch(user: Option[models.User], query: String, page: Int, itemsOnPage: Int): (Long, Seq[Entry]) = {
    require(itemsOnPage != 0)
